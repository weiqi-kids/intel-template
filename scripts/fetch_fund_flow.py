...
def _load_etf_definitions() -> dict[str, str]:
    """Load ETF definitions from companies.yml etf_benchmarks section."""
    config_path = BASE_DIR / "configs" / "companies.yml"
    if config_path.exists():
        import yaml
        with open(config_path, "r", encoding="utf-8") as f:
            config = yaml.safe_load(f) or {}
        etfs = config.get("etf_benchmarks", {})
        if etfs:
            return etfs
    return {"SPY": "S&P 500 ETF"}

ETF_DEFINITIONS = _load_etf_definitions()
