...
def _detect_industry() -> tuple[str, str]:
    """Detect industry code and Chinese name from configs or repo name."""
    repo_root = Path(__file__).parent.parent

    # Try configs/site.yml first
    site_yml = repo_root / "configs" / "site.yml"
    if site_yml.exists():
        with open(site_yml, "r", encoding="utf-8") as f:
            site = yaml.safe_load(f) or {}
        code = site.get("industry_code", "IND")
        name_zh = site.get("industry_name", "產業")
        return code, name_zh

    # Fallback: derive from repo directory name
    repo_name = repo_root.name
    code = repo_name.replace("-intel", "").upper()[:3]
    return code, code

INDUSTRY_CODE, INDUSTRY_ZH_NAME = _detect_industry()
